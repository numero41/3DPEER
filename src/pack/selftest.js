// Auto-tests sur le HTML PRODUIT : ré-extraction du payload et décodage
// complet avec le décodeur meshopt de three r160 — délibérément le plus
// conservateur du parc navigateur.
import fs from 'fs';
...
export async function selfTestGeo(output, ref) {
  ...
  if (!ok) throw new Error('auto-test geo : décodage non identique');
  console.log('auto-test geo (décodeur three r160 sur le HTML final) : OK');
}

export function selfTestGltf(output) {
  const c = unenvelope(extractPayload(fs.readFileSync(output, 'utf-8')));
  if (c.subarray(0, 4).toString() !== 'glTF') throw new Error('auto-test gltf : magic KO');
  ...
  console.log(`auto-test gltf : OK — meshes:${(j.meshes || []).length} skins:${(j.skins || []).length} anims:${(j.animations || []).length} morphs:${nt}`);
}
